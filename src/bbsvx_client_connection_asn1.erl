%%%-----------------------------------------------------------------------------
%%% @doc
%%% ASN.1 version of bbsvx_client_connection.
%%% Clean replacement using ASN.1 encoding instead of term_to_binary.
%%% @author yan
%%% @end
%%%-----------------------------------------------------------------------------

-module(bbsvx_client_connection_asn1).
...
connect(enter,
    ...
    of
        {ok, Socket} ->
            ...
            ConnectMsg = #header_connect{namespace = NameSpace, node_id = NodeId},
            case bbsvx_asn1_codec:encode_fast(ConnectMsg) of
                error ->
                    ?'log-error'("Failed to encode connect message"),
                    {stop, normal};
                EncodedMsg ->
...
connect(info,
    ...
    case bbsvx_asn1_codec:decode_fast(Bin) of
        error ->
            logger:error("Failed to decode connect response"),
            arc_event(NameSpace,
                      MyUlid,
                      #evt_connection_error{ulid = MyUlid,
                                            ...
                                            reason = decode_error}),
            {stop, normal};
        #header_connect_ack{node_id = TargetNodeId, result = Result} ->
            case Result of
                ok ->
                    case Type of
                        register ->
                            RegisterMsg = #header_register{namespace = NameSpace,
                                                         lock = CurrentLock,
                                                         ulid = MyUlid},
                            case bbsvx_asn1_codec:encode_fast(RegisterMsg) of
                                error ->
                                    {stop, normal};
                                EncodedMsg ->
                        ...
                        forward_join ->
                            ForwardJoinMsg = #header_forward_join{namespace = NameSpace,
                                                                ulid = MyUlid,
                                                                type = Type,
                                                                lock = CurrentLock,
                                                                options = Options},
                            case bbsvx_asn1_codec:encode_fast(ForwardJoinMsg) of
                                error ->
                                    {stop, normal};
                                EncodedMsg ->
                                    gen_tcp:send(Socket, EncodedMsg),
                        ...
                        JoinType when JoinType =:= mirror; JoinType =:= normal ->
                            NewLock = get_lock(?LOCK_SIZE),
                            gproc:set_value({n, l, {arc, out, MyUlid}}, NewLock),
                            JoinMsg = #header_join{namespace = NameSpace,
                                                 ulid = MyUlid,
                                                 current_lock = CurrentLock,
                                                 new_lock = NewLock,
                                                 type = JoinType,
                                                 options = Options},
                            case bbsvx_asn1_codec:encode_fast(JoinMsg) of
                                error ->
                                    {stop, normal};
                                EncodedMsg ->
                                    gen_tcp:send(Socket, EncodedMsg),
...
register(info,
    ...
    %% ASN.1 decoding instead of binary_to_term
    case bbsvx_asn1_codec:decode_fast(ConcatBuf) of
        error ->
            logger:error("Failed to decode register response"),
            arc_event(NameSpace,
                      MyUlid,
                      #evt_connection_error{ulid = MyUlid,
                                            direction = out,
        ...
        #header_register_ack{result = Result} ->
            case Result of
                ok ->
                    ?'log-info'("~p registering to ~p accepted",
                                [?MODULE, TargetNode]),
                    arc_event(NameSpace,
                              MyUlid,
                    ...
                    {next_state, connected, State#state{socket = Socket, buffer = <<>>}};
                Else ->
...
forward_join(info,
    ...
    case bbsvx_asn1_codec:decode_fast(ConcatBuf) of
        error ->
            logger:error("Failed to decode forward join response"),
            arc_event(NameSpace,
                      MyUlid,
                      #evt_connection_error{ulid = MyUlid,
                                            direction = out,
                                            ...
                                            node = TargetNode}),
            {stop, normal, State};
        #header_forward_join_ack{result = Result, type = Type} ->
            case Result of
                ok ->
        ...
        _Else ->
            logger:error("~p Invalid forward header received from ~p",
                         [?MODULE, TargetNode]),
            arc_event(NameSpace,
                      MyUlid,
                      #evt_connection_error{ulid = MyUlid,
...
forward_join(timeout,
             ...
             #state{namespace = NameSpace,
                    ...
                    ulid = Ulid} =
                 State) ->
    ?'log-error'("Forward join timeout to node : ~p", [TargetNode]),
    arc_event(NameSpace,
              Ulid,
              #evt_connection_error{ulid = Ulid,
                                    direction = out,
...
join(info,
    ...
    case bbsvx_asn1_codec:decode_fast(ConcatBuf) of
        error ->
            logger:error("Failed to decode join response"),
            arc_event(NameSpace,
                      MyUlid,
                      #evt_connection_error{ulid = MyUlid,
                                            direction = out,
...
connected(info, {send, Event}, #state{socket = Socket} = State)
    when Socket =/= undefined ->
    %% ASN.1 encoding instead of term_to_binary
    case bbsvx_asn1_codec:encode_fast(Event) of
        error -> 
            {keep_state, State};
        EncodedMsg ->
            gen_tcp:send(Socket, EncodedMsg),
...
parse_packet(Buffer,
    ...
    case bbsvx_asn1_codec:decode_fast(Buffer) of
        ...
        #ontology_history{} = Event ->
            ?'log-info'("Connection received Ontology history"),
            ontology_event(NameSpace, Event),
        ...
        #exchange_cancelled{} = Event ->
            ?'log-info'("Connection received Exchange cancelled"),
            arc_event(NameSpace, MyUlid, Event),
            {keep_state, State#state{buffer = <<>>}};
        #exchange_out{} = Event ->
            ?'log-info'("Client Connection received Exchange out"),
            arc_event(NameSpace, MyUlid, Event),
            {keep_state, State#state{buffer = <<>>}};
        #exchange_accept{} = Event ->
            arc_event(NameSpace, MyUlid, Event),
            {keep_state, State#state{buffer = <<>>}};
        #node_quitting{reason = Reason2} ->
            arc_event(NameSpace,
                      MyUlid,
            ...
            {keep_state, State#state{buffer = <<>>}};
        _Other ->
            ?'log-warning'("Unknown message received: ~p", [_Other]),
            {keep_state, State#state{buffer = <<>>}}
    end.
