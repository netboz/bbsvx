...
initialize_ontology(
    ...
    case load_history(OntState, State#state.repos_table) of
        #ont_state{} = NewOntState ->
            ?'log-info'(
                "Ontology Agent ~p loaded history as root local index: ~p current "
                "index: ~p",
                [
                    Namespace,
            ...
            {keep_state, State#state{
                ont_state = NewOntState#ont_state{current_index = 0}, boot = root
            }};
        {error, Reason} ->
...
initialize_ontology(
    ...
    case load_history(OntState, State#state.repos_table) of
        #ont_state{} = NewOntState ->
            ?'log-info'(
                "Ontology Agent ~p loaded history as join, local index: ~p current "
                "index: ~p",
                [
                    Namespace,
                    NewOntState#ont_state.local_index,
...
wait_for_registration(
    ...
    case {CurrentIndex, LocalIndex} of
        ...
        _ ->
            ?'log-error'(
                "Current index ~p is lower than local index ~p",
...
retrieve_transaction_history(Namespace, OldestIndex, YoungerIndex) ->
    ...
    SelectFun =
        ets:fun2ms(fun(#transaction{index = Index} = Transaction) when
