...
start_link(Namespace, Options) ->
    gen_statem:start({via, gproc, {n, l, {?SERVER, Namespace}}},
                     ?MODULE,
                     [Namespace, Options],
                     []).
...
init([Namespace, Options]) ->
    ...
    ?'log-info'("Epto dissemination component starting ~p with options ~p",
                [Namespace, Options]),
...
order_events(Ball, Delivered, Received, LastDeliveredTs, LogicalClock, CurrentIndex) ->
                     ...
                     case ordsets:is_element(BallEvtId, Delivered) of
                         false when Ts >= LastDeliveredTs ->
                             case maps:get(BallEvtId, ReceivedEvts, undefined) of
                                 ...
                                 #epto_event{ttl = ReceivedEvtTtl} = ReceivedEvt
                                     when ReceivedEvtTtl < BallEvtTtl ->
                                     ReceivedEvts#{BallEvtId =>
    ...
    {DeliverableEvents, UpdatedMinQueueTs} =
        maps:fold(fun(_EvtId, #epto_event{ts = EvtTs} = Evt, {DeliverableEvents, MQTs}) ->
    ...
    FilteredDeliverableEvent =
        lists:filter(fun (#epto_event{ts = Ts}) when Ts > UpdatedMinQueueTs ->
                             false;
                         (_) ->
                             true
                     end,
    ...
    FilteredReceivedEvents =
        lists:foldl(fun(#epto_event{id = DelivEvtId}, AccReceived) ->
    ...
    {NewDeliveredEvents, NewLastDeliveredTs, NewIndex} =
        lists:foldl(fun (#epto_event{ts = EvtTs, id = EvtId} = Evt,
