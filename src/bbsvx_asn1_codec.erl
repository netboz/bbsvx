        ...
        case decode_message(Binary) of
            ...
            {error, _} ->
                ...
                try
                    ...
                    % Return binary if can't parse
                    _:_ -> Binary
                end
        end
    catch
        % Return binary if can't decode
        _:_ -> Binary
    end.
...
erlang_to_asn1(#header_register_ack{result = Result, leader = Leader, current_index = Index}) ->
    ...
    {headerRegisterAck, #'HeaderRegisterAck'{
        result = ASN1Result, leader = ASN1Leader, currentIndex = ASN1Index
    }};
%% Join operations
erlang_to_asn1(#header_join{
    namespace = Namespace,
    ulid = Ulid,
    type = Type,
...
%% Forward join
erlang_to_asn1(#header_forward_join{
    namespace = Namespace,
    ulid = Ulid,
    type = Type,
    lock = Lock,
...
erlang_to_asn1(#ontology_history{
    namespace = Namespace,
...
erlang_to_asn1(#ontology_history_request{
    namespace = Namespace,
    requester = Requester,
    oldest_index = OldestIndex,
    younger_index = YoungerIndex
...
erlang_to_asn1(#leader_election_info{
    payload = #neighbor{
        node_id = NodeId,
...
erlang_to_asn1(#transaction{
    ...
    {'Transaction', Index, ASN1Type, Signature, TsCreated, TsPosted, TsDelivered, ASN1TsProcessed,
        SourceOntologyId, PrevAddress, PrevHash, CurrentAddress, Namespace, ASN1Leader,
...
asn1_to_erlang({headerConnect, Map}) when is_map(Map) ->
    ...
    VersionStr = maps:get(version, Map, "1.0.0"),
    ...
    NodeId = maps:get(nodeId, Map, undefined),
...
asn1_to_erlang(
    {headerRegisterAck, #'HeaderRegisterAck'{
        result = Result, leader = Leader, currentIndex = Index
    }}
) ->
    ErlResult =
        case Result of
...
asn1_to_erlang(
    ...
    #header_join{
        namespace = Namespace,
        ulid = Ulid,
        type = erlang:binary_to_atom(Type),
        current_lock = CurrentLock,
        new_lock = NewLock,
        options = ErlOptions
    };
asn1_to_erlang(
    {headerJoinAck, #'HeaderJoinAck'{result = Result, joinType = Type, options = Options}}
) ->
    ErlResult =
        case Result of
            ok -> ok;
...
asn1_to_erlang(
    {headerForwardJoin, #'HeaderForwardJoin'{
        namespace = Namespace, ulid = Ulid, joinType = Type, lock = Lock, options = Options
    }}
) ->
    ErlOptions =
        case Options of
...
asn1_to_erlang({nodeQuitting, Map}) when is_map(Map) ->
    ...
    #node_quitting{reason = ErlReason};
asn1_to_erlang(
    {ontologyHistory, {'OntologyHistory', Namespace, OldestIndex, YoungerIndex, SerializedTx}}
) ->
...
asn1_to_erlang(
    {ontologyHistoryRequest,
        {'OntologyHistoryRequest', Namespace, SerializedRequester, OldestIndex, YoungerIndex}}
) ->
    Requester =
        case SerializedRequester of
            asn1_NOVALUE -> undefined;
            undefined -> undefined;
...
asn1_to_erlang(
    {leaderElectionInfo,
        {'LeaderElectionInfo', {'Neighbor', NodeId, ChosenLeader, PublicKey, SignedTs, Ts}}}
) ->
    Neighbor = #neighbor{
        node_id = NodeId,
        chosen_leader = ChosenLeader,
    ...
    ASN1Host =
        case Host of
            ...
            H when is_tuple(H), tuple_size(H) =:= 8 ->
                %% IPv6 tuple
                {ipv6, <<<<X:16>> || X <- tuple_to_list(H)>>};
            H ->
                {hostname, list_to_binary(io_lib:format("~p", [H]))}
        end,
...
convert_exchange_entry(#exchange_entry{
    ulid = Ulid, lock = Lock, target = Target, new_lock = NewLock
}) ->
