%%%-----------------------------------------------------------------------------
%%% BBSvx Erlog Database Differ
%%%-----------------------------------------------------------------------------

-module(bbsvx_erlog_db_differ).

-moduledoc "BBSvx Erlog Database Differ\n\n"
"Erlog database module proxy that logs all database modification operations.\n\n"
"Acts as a proxy for another erlog db module, tracking changes for transaction diffs.".
...
%% new(InitArgs) -> Db.
-doc "add_built_in(Db, Functor) -> Db. Add functor as a built-in in the database.".
-callback new(InitArgs :: term()) -> term().
%% add_built_in(Db, Functor) -> Db.
%%  Add functor as a built-in in the database.
-doc """
add_compiled_code(Db, Functor, Module, Function) -> \{ok,Db\} | error. Add functor as a compiled procedure with code in M:F in the database. Check that it is not a built-in, if so return error.
""".
...
-doc """
asserta_clause(Db, Functor, Head, Body) -> \{ok,NewDb\} | error. assertz_clause(Db, Functor, Head, Body) -> \{ok,NewDb\} | error. Insert a clause at the beginning or end of the database.
""".
...
-doc """
retract_clause(Db, Functor, ClauseTag) -> \{ok,NewDb\} | error. Retract (remove) the clause with tag ClauseTag from the list of clauses of Functor.
""".
...
-doc """
get_procedure(Db, Functor) -> built_in | \{code,\{Mod,Func\}\} | \{clauses,\[Clause]\} | undefined. Return the procedure type and data for a functor.
""".
...
-doc "get_procedure_type(Db, Functor) -> built_in | compiled | interpreted | undefined. Return the procedure type for a functor.".
