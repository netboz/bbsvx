...
-export([handle_register_user/2, handle_mqtt_publish_hook/3, handle_mqtt_subscribe_hook/4,
         handle_mqtt_unsubscribe_hook/2, handle_sm_register_connection_hook/3, handle_sm_remove_connection_hook/3, handle_remove_user_hook/2]).
...
start(Host, Opts) ->
    ...
    ejabberd_hooks:add(register_user, Host, ?MODULE, handle_register_user, 10),
    ejabberd_hooks:add(mqtt_publish, Host, ?MODULE, handle_mqtt_publish_hook, 10),
    ejabberd_hooks:add(mqtt_subscribe, Host, ?MODULE, handle_mqtt_subscribe_hook, 10),
    ejabberd_hooks:add(mqtt_unsubscribe, Host, ?MODULE, handle_mqtt_unsubscribe_hook, 10),
    %% Add hook on connection openning
    ejabberd_hooks:add(sm_register_connection_hook,
                       Host,
                       ?MODULE,
                       handle_sm_register_connection_hook,
                       10),
    %% Add hook on connection closing
    ejabberd_hooks:add(sm_remove_connection_hook,
                       Host,
                       ?MODULE,
                       handle_sm_remove_connection_hook,
                       100),
    ejabberd_hooks:add(remove_user, Host, ?MODULE, handle_remove_user_hook, 10),
    ok.
...
handle_sm_remove_connection_hook(SID, JID, Info) ->
    logger:info("BBSVX ejabberd mod : Connection removed ~p ~p ~p", [SID, JID, Info]),
    ok.

handle_sm_register_connection_hook(SID, JID, Info) ->
    logger:info("BBSVX ejabberd mod : Connection registered ~p ~p ~p", [SID, JID, Info]),
    ok.

handle_remove_user_hook(LUser, LServer) ->
    logger:info("BBSVX ejabberd mod : Remove user ~p ~p", [LUser, LServer]),
    %% Notify spray agent
    gproc:send({n, l, {bbsvx_actor_spray_view, <<"ont_test">>}}, {remove_node, LUser}),
    ok.
...
process_publish(_Usr,
                ...
                {subscribe,
                 #node_entry{node_id = RequesterClientId,
                             host = RequesterHost,
                             port = RequesterPort} =
                     RequesterNode}) ->
    logger:info("BBSVX ejabberd mod : Got ontology : ~p subscribe request from: "
                "~p ~p",
    ...
    gproc:send({p, l, {ontology, Namespace}}, {contact_request, RequesterNode}),
...
process_publish(_Usr,
                [<<"ontologies">>, <<"in">>, Namespace, _FromClientId],
...
process_unsubscribe(Usr, Topic) ->
    logger:info("BBSVX ejabberd mod : Process_unsubscribe called with Usr: ~p, "
                "Topic: ~p",
                [Usr, Topic]),
    ok.
